...
RTA.clients.synologyAdder = function(server, torrentdata, torrentname) {
	...
	xhr.open("GET", scheme + server.host + addPort(server) + "/webapi/auth.cgi?api=SYNO.API.Auth&version=2&method=login&account=" + server.login + "&passwd=" + server.password + "&session=DownloadStation&format=sid", false);
	xhr.send(null);
	var sid;
	...
	if(torrentdata.substring(0,7) == "magnet:") {
		...
		console.log("GET: " + scheme + server.host + addPort(server) + "/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&_sid=" + sid + "&uri=" + encodeURIComponent(torrentdata));
		var mxhr = new XMLHttpRequest();
		mxhr.open("GET", scheme + server.host + addPort(server) + "/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&_sid=" + sid + "&uri=" + encodeURIComponent(torrentdata), true);
		mxhr.onreadystatechange = handleResponse;
		mxhr.send(message);
	} else {
		var xhr = new XMLHttpRequest();
		xhr.open("POST", scheme + server.host + addPort(server) + "/webapi/DownloadStation/task.cgi?api=SYNO.DownloadStation.Task&version=2&method=create&_sid=" + sid, true);
