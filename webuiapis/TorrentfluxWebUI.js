RTA.clients.torrentfluxAdder = function(server, torrentdata, torrentname) {
	...
	var loginurl = "http" + (server.hostsecure ? "s" : "") + "://" + server.host + addPort(server) + server.torrentfluxrelativepath + "/login.php";
	...
	// send the torrent
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "http" + (server.hostsecure ? "s" : "") + "://" + server.host + addPort(server) + server.torrentfluxrelativepath + "/index.php", true);
